document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterProjects(category) {
    // Reset any inline positioning from masonry so grid can reflow
    ...
    projectCards.forEach((card) => {
      ...
      if (category === "all" || cardCategory === category) {
        // revert to stylesheet display (let grid handle layout)
        card.style.display = "";
    ...
    // After changing visibility, recompute masonry/layout for visible items
    // Use a small timeout to allow DOM reflow
  ...
  function applyMasonryLayout() {
    ...
    if (window.innerWidth >= 768) {
      ...
      // Consider only visible items for layout
      ...
      const firstItemWidth =
        items[0].getBoundingClientRect().width || grid.clientWidth;
      const columnCount = Math.max(
        1,
        Math.floor(grid.clientWidth / firstItemWidth),
      );
  ...
  function showProjectModal(title, description) {
    ...
    modalNote.textContent =
      "This is a demonstration modal for the project preview functionality.";
